...
func (s *Server) Router(rdb *redis.Client) http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/admin/offers/{offerId}/domains", s.AdminOfferDomainsList)
			r.Post("/admin/offers/{offerId}/domains", s.AdminOfferDomainCreate)
			r.Patch("/admin/offers/{offerId}/domains/{id}", s.AdminOfferDomainUpdate)
			r.Delete("/admin/offers/{offerId}/domains/{id}", s.AdminOfferDomainDelete)
